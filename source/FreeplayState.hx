...
class FreeplayState extends MusicBeatState
{
	...
	function fakeloading():Void
	{
		var black:FlxSprite = new FlxSprite(-200, -100).makeGraphic(FlxG.width * 2, FlxG.height * 2, FlxColor.BLACK);
		add(black);

		var loadingbg:FlxSprite = new FlxSprite(0, 0);
		loadingbg.loadGraphic(Paths.image('Loading'));
		loadingbg.antialiasing = FlxG.save.data.antialiasing;
		loadingbg.setGraphicSize(Std.int(loadingbg.width * 0.67));
		loadingbg.updateHitbox();
		loadingbg.alpha = 0;
		add(loadingbg);
		FlxTween.tween(loadingbg, {alpha: 1}, 0.5);		
	}
}
