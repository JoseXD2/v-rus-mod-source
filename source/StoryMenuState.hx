...
class StoryMenuState extends MusicBeatState
{
	...
	{
		return [
			['Malfunction','Invade','R Memory','WARNING','The battle of robbery'],
			['Virus R']
		];
	}
	...
	var menuR_1:FlxSprite = new FlxSprite(0,56).loadGraphic(Paths.image('campaignmenucharacter_R_Week1'));
	var menuR_2:FlxSprite = new FlxSprite(0,56).loadGraphic(Paths.image('campaignmenucharacter_R_Week2'));
		...
		if (!movedBack)
		{
			...
			if (controls.ACCEPT)
			{
				...
				{	
					...
					add(black);	
					var loading:FlxSprite = new FlxSprite(0,0).loadGraphic(Paths.image('Loading'));		
					loading.setGraphicSize(Std.int(loading.width * 0.67));
					loading.updateHitbox();
					loading.antialiasing = FlxG.save.data.antialiasing;
					loading.alpha = 0;
					add(loading);
	...
	function updateText()
	{
		...
		{
			if(!menuR_1.visible)
				menuR_1.visible = true;
		}
		else
		{
			if(menuR_1.visible)
				menuR_1.visible = false;
		}
	}
