...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch(songLowercase)
		{
			...
			case 'malfunction'|'invade'|'warning':
				if (FlxG.save.data.language)			
					dialogue = CoolUtil.coolTextFile(Paths.txt('data/' + songLowercase + '/dialogue-zh'));								
				else		
					dialogue = CoolUtil.coolTextFile(Paths.txt('data/' + songLowercase + '/dialogue-en'));		
			case 'the-battle-of-robbery':
				if (FlxG.save.data.language)			
					dialogueEnd = CoolUtil.coolTextFile(Paths.txt('data/' + songLowercase + '/dialogueEnd-zh'));							
				else		
					dialogueEnd = CoolUtil.coolTextFile(Paths.txt('data/' + songLowercase + '/dialogueEnd-en'));
			case 'r-memory'|'virus-r':
				if (FlxG.save.data.language)
				{
					dialogue = CoolUtil.coolTextFile(Paths.txt('data/' + songLowercase + '/dialogue-zh'));
					dialogueEnd = CoolUtil.coolTextFile(Paths.txt('data/' + songLowercase + '/dialogueEnd-zh'));
		...
		if (SONG.stage == null)
		{
			switch (songLowercase)
			{
				case 'invade' | 'malfunction' |'r-memory':
					stageCheck = 'cyber';
				case 'warning'|'the-battle-of-robbery':
					stageCheck = 'cyber2';
				case 'virus-r':
					stageCheck = 'window';
				default:
		...
		if (!PlayStateChangeables.Optimize)
		{
			switch (stageCheck)
			{
				case 'cyber':
					...
					cyberwall = new FlxSprite(-557, -298);
					cyberwall.frames = Paths.getSparrowAtlas('stage/cyber/wall','r');
					cyberwall.animation.addByPrefix('idle', "wall", 24);
					cyberwall.animation.play('idle');
					cyberwall.scrollFactor.set(0.8, 0.8);
					cyberwall.antialiasing = FlxG.save.data.antialiasing;				
					add(cyberwall);

					var floor:FlxSprite = new FlxSprite(-17, 726).loadGraphic(Paths.image('stage/cyber/floor','r'));
					floor.scrollFactor.set(1, 1);
					add(floor);

					var tv:FlxSprite = new FlxSprite(237,253).loadGraphic(Paths.image('stage/cyber/TV','r'));
					tv.scrollFactor.set(1, 1);
					add(tv);

					tvL = new FlxSprite(-583, -20);
					tvL.frames = Paths.getSparrowAtlas('stage/cyber/TV left','r');
					tvL.animation.addByPrefix('idle', "TV left", 24);
					tvL.animation.play('idle');
					tvL.scrollFactor.set(1, 1);
					tvL.antialiasing = FlxG.save.data.antialiasing;
					add(tvL);

					tvR = new FlxSprite(1238, -67);
					tvR.frames = Paths.getSparrowAtlas('stage/cyber/TV right','r');
					tvR.animation.addByPrefix('idle', "TV right", 24);
					tvR.animation.play('idle');
					tvR.scrollFactor.set(1, 1);
					tvR.antialiasing = FlxG.save.data.antialiasing;
					add(tvR);

					screen = new FlxSprite(-603,-265).loadGraphic(Paths.image('stage/cyber/Screen','r'));
					screen.scrollFactor.set(1.2, 1.2);
					
					line = new FlxSprite(-542,-281).loadGraphic(Paths.image('stage/cyber/line','r'));
					line.scrollFactor.set(1.2, 1.2);

				case 'cyber2':
					curStage = 'cyber2';
					defaultCamZoom = 0.73;

					var bottom:FlxSprite = new FlxSprite(0,0).loadGraphic(Paths.image('stage/cyber2/bottomcolor','r'));
					add(bottom);

					var behind:FlxSprite = new FlxSprite(0,0).loadGraphic(Paths.image('stage/cyber2/behindwall','r'));
					add(behind);

					var wall:FlxSprite = new FlxSprite(0,0).loadGraphic(Paths.image('stage/cyber2/wall','r'));
					add(wall);

					var floor:FlxSprite = new FlxSprite(0,0).loadGraphic(Paths.image('stage/cyber2/floor','r'));
					add(floor);

					var tv:FlxSprite = new FlxSprite(0,0).loadGraphic(Paths.image('stage/cyber2/TV','r'));
					add(tv);

					screen = new FlxSprite(0,0).loadGraphic(Paths.image('stage/cyber2/screen','r'));
				//	screen.scrollFactor.set(1.2, 1.2);
					
					line = new FlxSprite(0,0).loadGraphic(Paths.image('stage/cyber2/blueline','r'));
				//	line.scrollFactor.set(1.2, 1.2);

					mute = new FlxSprite(0,0);
					mute.frames = Paths.getSparrowAtlas('stage/cyber2/mute','r');
					mute.animation.addByPrefix('idle', 'jingyin', 24, false);
					mute.cameras = [camHUD];
					mute.setGraphicSize(Std.int(mute.width * 0.5));
					mute.updateHitbox();
					mute.alpha = 0;

					blue = new FlxSprite(0,0);
					blue.frames = Paths.getSparrowAtlas('stage/cyber2/blue','r');
					blue.animation.addByPrefix('idle', 'texiao', 24, true);
				...
				case 'window':
					...
					var white:FlxSprite = new FlxSprite(0,0).makeGraphic(1900, 1200, FlxColor.WHITE);
					add(white);

					winbg = new FlxSprite(0,0);
					winbg.frames = Paths.getSparrowAtlas('stage/window/week2BG','r');
					winbg.animation.addByPrefix('idle', 'week2Background', 24, false);
					winbg.antialiasing = FlxG.save.data.antialiasing;

					win1 = new FlxSprite(0,0).loadGraphic(Paths.image('stage/window/window_1','r'));
					win1.antialiasing = FlxG.save.data.antialiasing;

					line = new FlxSprite(0,0);
					line.frames = Paths.getSparrowAtlas('stage/window/tiaowen','r');
					line.animation.addByPrefix('idle', 'tiaowen', 24, false);
					line.antialiasing = FlxG.save.data.antialiasing;

					win2 = new FlxSprite(0,0).loadGraphic(Paths.image('stage/window/window_2','r'));
					win2.antialiasing = FlxG.save.data.antialiasing;

					warning = new FlxSprite(0,0);
					warning.frames = Paths.getSparrowAtlas('stage/window/Warning','r');
					warning.animation.addByPrefix('idle', 'warning', 24, false);
					warning.antialiasing = FlxG.save.data.antialiasing;

					specialr = new FlxSprite(10,14);
					specialr.frames = Paths.getSparrowAtlas('stage/window/Rteshu','r');
					specialr.animation.addByPrefix('black', 'Rblack', 24, false);
					specialr.animation.addByPrefix('smile', 'Rsmile', 18, false);
					specialr.antialiasing = FlxG.save.data.antialiasing;
					specialr.alpha = 0;

					border = new FlxSprite(0,0);
					border.frames = Paths.getSparrowAtlas('stage/window/Black','r');
					border.animation.addByPrefix('idle', 'Black', 12, false);
					border.antialiasing = FlxG.save.data.antialiasing;
	...
	//	gf.scrollFactor.set(0.95, 0.95);
		...
		switch (SONG.player2)
		{
			case 'r'|'r3'|'r-window'|'r-window-special':
				healthBar.createFilledBar(0xFFC33647, 0xFF3DB0CF);
			case 'acesora':
				healthBar.createFilledBar(0xFF86D9FF, 0xFF3DB0CF);
			default:
	...
	function truediaEnd(?dialogueBox:DialogueBox):Void
	{
		inCutscene = true;					
		add(dialogueBox);
	}

	function diaEnd():Void
	{
		endingSong = true;
		inCutscene = true;
		startingSong = true;
	...
	public function generateSong(dataPath:String):Void
	{
		...
		switch(songLowercase)
		{
			case 'r-memory'|'the-battle-of-robbery'|'virus-r':
				if(isStoryMode)
					FlxG.sound.music.onComplete = diaEnd;
				else
	...
	private function generateStaticArrows(player:Int):Void
	{
		for (i in 0...4)
		{
			...
			babyArrow.x += 50;
			babyArrow.x += ((FlxG.width / 2 + 85) * player);
	...
	override public function update(elapsed:Float)
	{
		...
		if (generatedMusic && PlayState.SONG.notes[Std.int(curStep / 16)] != null)
		{
			...
			if (camFollow.x != dad.getMidpoint().x + 150 && !PlayState.SONG.notes[Std.int(curStep / 16)].mustHitSection && curStage != 'window')
			{
				var offsetX = 0;
				var offsetY = 0;
				#if cpp
			...
			if (PlayState.SONG.notes[Std.int(curStep / 16)].mustHitSection && camFollow.x != boyfriend.getMidpoint().x - 100 && curStage != 'window')
			{
				var offsetX = 0;
				var offsetY = 0;
				#if cpp
		...
		if (generatedMusic)
		{
			...
			notes.forEachAlive(function(daNote:Note)
			{
				...
				if (!daNote.mustPress && daNote.wasGoodHit)
				{
					...
					if (songLowercase == 'the-battle-of-robbery' && dad.curCharacter == 'acesora' && specialmove)
						altAnim = '-alt';
					...
					if (!daNote.isParent && daNote.parent != null)
					{
						if (daNote.spotInLine != daNote.parent.children.length - 1)
						{
							...
							if (daNote.isSustainNote && (dad.curCharacter == 'r3'|| dad.curCharacter == 'r-window'))
								dad.playAnim('sing' + dataSuffix[singData] + 'Sus', true);
							else
	...
	override function stepHit()
	{
		...
		if (songLowercase == 'the-battle-of-robbery' && dad.curCharacter == 'acesora')
		{
			switch (curStep)
			{
				case 580:
		...
		if (songLowercase == 'virus-r' && dad.curCharacter == 'r-window')
		{
			switch (curStep)
			{
				case 239:
					dad.alpha = 0;
					specialr.alpha = 1;
					specialr.animation.play('black',true);
				case 256:
					dad.alpha = 1;
					specialr.alpha = 0;
				case 768:
					border.animation.play('idle',true,true);
					border.alpha = 1;
				case 1471:
					FlxTween.tween(border, {alpha: 0}, 0.5);
				//	border.alpha = 0;
				case 1776:
					dad.alpha = 0;
					specialr.alpha = 1;
				...
				case 2692:
					border.animation.play('idle',true,true);
					border.alpha = 1;
					FlxTween.tween(border, {alpha: 0}, 19);
				case 2815:
					dad.alpha = 0;
					specialr.alpha = 1;
					specialr.animation.play('smile',true);
			}
		}
	}
	...
	override function beatHit()
	{
		...
		if (curSong == 'Invade' && curBeat == 226 && dad.curCharacter == 'r')
		{
			dad.playAnim('angry');
		}

		if (curSong == 'R-Memory' && curBeat % 4 == 0 && dad.curCharacter == 'r3')
		{
			if (!isup)
			{
				FlxTween.tween(dad,{y: 125},60 / SONG.bpm * 2,{ease: FlxEase.sineOut,
				onComplete:function(twn:FlxTween)
				{
						FlxTween.tween(dad,{y: 95},60 / SONG.bpm * 2,{ease: FlxEase.sineIn});
						isup = true;
				}});
			}
			else
			{
				FlxTween.tween(dad,{y: 65},60 / SONG.bpm * 2,{ease: FlxEase.sineOut,
					onComplete:function(twn:FlxTween)
					{
							FlxTween.tween(dad,{y: 95},60 / SONG.bpm * 2,{ease: FlxEase.sineIn});
							isup = false;
		...
		if (!boyfriend.animation.curAnim.name.startsWith("sing") && (curBeat % idleBeat == 0 || !idleToBeat) && !inCutscene)
		{
		...
		{
			case 'cyber2':
				mute.animation.play('idle',true);
			case 'window':
				winbg.animation.play('idle',true);
				line.animation.play('idle',true);
